...
class Authlite_Core {
	...
	public function __construct($config_name = 'authlite')
	{
		...
		$this->email_column	   = $this->config['email'];
	...
	public function login($username, $password, $remember = false)
	{
		...
		// Tweak to allow login by username or email
		// First find records with the given username or password
		$user = ORM::factory($this->user_model)->where(array(
			$this->username_column => $username,
			$this->password_column => $this->hash($password)
		))->find();

		// No entry in username column, look for email
		if ( ! $user->loaded)
		{
			Kohana::log('info', 'Checking credentials using email...');

			$user = ORM::factory($this->user_model)
						->where(array(
							$this->email_column => $username,
							$this->password_column => $this->hash($password)
					))->find();
		}
